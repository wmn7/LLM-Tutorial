...
LastEditTime: 2025-04-10 16:55:39
LastEditors: Maonan Wang
Description: 两个 Agent 按照顺序执行. (1) 第一个 agent 理解图片; (2) 第二个 agent 根据场景判断做的内容
FilePath: /llm_tutorial/QwenAgent-Tutorial/4_sequence_agents.py
'''
import copy
...
class TSCDecision(Agent):
    ...
    def _run(self, messages: List[Message], lang: str = 'zh', **kwargs) -> Iterator[List[Message]]:
        ...
        new_messages[-1]['content'].append(ContentItem(text='这是一个路口某个方向的摄像头图片，请你仔细观察该路口是否包含特殊车辆，还是常规场景。'))
        response = []
...
def app_tui():
    bot = TSCDecision(
        ...
        function_list=['tsc_expert_decision']
    )
